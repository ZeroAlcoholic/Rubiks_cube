...
支援 3×3、4×4、5×5 三種尺寸，部署於 GitHub Pages，可安裝為 PWA。
...
- **PWA**：`manifest.json` + `service-worker.js`，支援「加入主畫面」安裝
...
- **不在 iOS PWA Standalone 模式依賴鏡頭**：已知 iOS 14 bug，引導使用者用 Safari 瀏覽器開啟
...
Phase 8 → PWA + 入口頁 + GitHub Pages 部署
```
...
8. **手機實測**：iPhone Safari + Android Chrome 各別測試鏡頭授權與 PWA 安裝
